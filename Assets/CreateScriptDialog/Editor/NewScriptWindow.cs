...
public class NewScriptWindow : EditorWindow
{
    ...
    public static void Init(GameObject objectToAttatchTo)
    {
        GetWindow<NewScriptWindow>(true, "Create Script");
        activeObject = objectToAttatchTo;
    }
	...
	*/
	...
	private void Create ()
	{
        ...
        if (CanAddComponent())
        {
            // Use EditorPrefs to save information between AssetDatabase Refresh
            EditorPrefs.SetBool("waitingOnAttatch", true);
            string tPath = TargetPath();
            EditorPrefs.SetString("tPath", tPath);
            EditorPrefs.SetInt("objToAttatchTo", m_GameObjectToAddTo.GetInstanceID());
    ...
    {
        bool waitingOnAttatch = EditorPrefs.GetBool("waitingOnAttatch");

        if (waitingOnAttatch)
        {
            EditorPrefs.SetBool("waitingOnAttatch", false);
            string tPath = EditorPrefs.GetString("tPath");
            int instId = EditorPrefs.GetInt("objToAttatchTo");

            GameObject objToAttatchTo = EditorUtility.InstanceIDToObject(instId) as GameObject;

            MonoScript file = AssetDatabase.LoadAssetAtPath(tPath, typeof(MonoScript)) as MonoScript;
            objToAttatchTo.AddComponent((file).GetClass());
        }
    }
    ...
    private void CustomAttachToGUI()
    {
        GUILayout.BeginHorizontal();
        {
